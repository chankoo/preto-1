...
for i, name in enumerate(corp_names, start=1):
    ...
    corporation_entries.append(
        {
            "CORP_ID": corp_id,
            "CORP_REL_START_DATE": rel_start_date,
            "CORP_NAME": name,
            "UP_CORP_ID": None,
            "CORP_USE_YN": "Y",
            "CORP_REL_END_DATE": None,
            "CORP_REG_LEVEL": "Corporation",
        }
    )
...
for name in selected_branches:
    ...
    start_dates = sorted(
        [random_date(start_date, end_date).date() for _ in range(num_assignments)]
    )
    for i in range(num_assignments):
        ...
        branches.append(
            {
                "CORP_ID": branch_id,
                "CORP_REL_START_DATE": start_dates[i],
                "CORP_NAME": f"{assigned_corp['CORP_NAME']} {name}",
                "UP_CORP_ID": assigned_corp["CORP_ID"],
                "CORP_USE_YN": "Y",
                "CORP_REL_END_DATE": None,
                "CORP_REG_LEVEL": "Branch",
            }
        )
    branch_id_counter += 1

corp_branch_df = pd.DataFrame(corporation_entries + branches)
corp_branch_df["SORT_KEY"] = corp_branch_df["CORP_REG_LEVEL"].apply(
    lambda x: 0 if x == "Corporation" else 1
)
corp_branch_df = corp_branch_df.sort_values(
    by=["SORT_KEY", "CORP_ID", "CORP_REL_START_DATE"]
...
corp_branch_df["CORP_REL_START_DATE"] = pd.to_datetime(
    corp_branch_df["CORP_REL_START_DATE"]
)
corp_branch_df["CORP_REL_END_DATE"] = pd.to_datetime(
    corp_branch_df["CORP_REL_END_DATE"], errors="coerce"
)
...
corp_branch_df_for_gsheet["CORP_REL_START_DATE"] = corp_branch_df_for_gsheet[
    "CORP_REL_START_DATE"
].dt.strftime("%Y-%m-%d")
corp_branch_df_for_gsheet["CORP_REL_END_DATE"] = corp_branch_df_for_gsheet[
    "CORP_REL_END_DATE"
].dt.strftime("%Y-%m-%d")
...
corp_branch_df_for_gsheet = corp_branch_df_for_gsheet.replace(
    {"None": "", "nan": "", "NaT": ""}
)
